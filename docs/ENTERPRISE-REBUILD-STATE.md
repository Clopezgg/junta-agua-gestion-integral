...
| Campo | Valor |
|---|---|
| CURRENT_PHASE | Milestone P — Cumplimiento (EN PROGRESO): ERSAPS/Calendario/Transparencia en DS. Falta: Reports/Annual Pack (dashboard financiero legacy). A–O integrados. |
| CURRENT_BRANCH | `work/junta-enterprise-rebuild` |
| CURRENT_SHA | `pending-push` (Milestone P parcial) sobre `0fc6d89` |
| LAST_GREEN_SHA | `0fc6d89` (CI PR #22 en curso; `1418d7b` verde 4/4) — 256 tests, lint OK, tsc OK, build OK |
| LAST_COMPLETED_MILESTONE | O — Junta / Gobierno (JuntaDirectiva persona-picker + Asamblea/Reuniones/Resoluciones/Proyectos/Comités en DS). Antes: N, M, L, K, J, I, H. |
| NEXT_ACTION | Cerrar P: rediseñar `Reports.tsx` (Annual Pack / Reporting §P) sobre design system, mantener export XLS/PDF y evidencias. Luego Q (Documentos). Draft PR: #22. |
| TEST_COUNT | 259 (vitest) — +gobierno (O) +cumplimiento (P: DS, sin prompt, sin claims falsos) |
...
| P — Cumplimiento (ERSAPS / Calendario / Transparencia) | 🔶 EN PROGRESO | `pending-push` | Ersaps/Calendario/Transparencia sobre design system, sin `window.prompt`; ERSAPS mantiene "registrar ≠ cumplir + validación institucional" (sin claims falsos). Suite `compliance-milestone-p`. Falta: `Reports.tsx` (Annual Pack / Reporting) — dashboard financiero legacy pendiente de rediseño. |
