...
| Campo | Valor |
|---|---|
| CURRENT_PHASE | Milestone V — Legacy eradication (EN PROGRESO): 3 `.tsx` legacy restantes (subscriber/portal, se cierran con R). Q/R/S/T/U/W/X/Y/Z pendientes. A–P integrados. |
| CURRENT_BRANCH | `work/junta-enterprise-rebuild` |
| CURRENT_SHA | `pending-push` (Milestone V lotes 1-5: 20 pantallas legacy → DS, Layout muerto eliminado) sobre `879fea0` |
...
| NEXT_ACTION | Milestone R (portal DNI→foto→nacimiento, sin contraseña) + cierre de las 3 pantallas subscriber/portal legacy; luego Q (Centro documental), S (dedup rutas admin), T/U/W/X/Y/Z. Draft PR: #22. |
| TEST_COUNT | 260 (vitest) — V: 20 pantallas legacy migradas, gates limpios, suites V2/V3 reapuntadas a AppShell |
...
| LEGACY_FILES_REMAINING | 0 CSS legacy en `src/` · **3** `.tsx` en `docs/legacy-ui-allowlist.txt` (SubscriberCards, SubscriberPortal, Subscribers — se cierran en R junto al portal sin contraseña; Subscribers/SubscriberCards son además duplicados de AbonadosList/Abonado360) · **0** en `docs/legacy-uuid-allowlist.txt`. `src/components/Layout.tsx` eliminado (código muerto). El gate impide que las listas crezcan o queden entradas obsoletas. |
...
| O — Junta / Gobierno | ✅ COMPLETE | `0fc6d89` | JuntaDirectiva: persona-picker del registro maestro (mig. 015 `list_governance_persons` + alta inline) — allowlist UUID vacío (§64). Asamblea/Reuniones/Resoluciones/Proyectos/Comités sobre design system, actas y comités en Dialog (sin `window.prompt`). Cargo institucional ≠ rol del sistema. Suite `governance-milestone-o`. |
| P — Cumplimiento (ERSAPS / Calendario / Transparencia / Reports) | ✅ COMPLETE | `879fea0` | Ersaps/Calendario/Transparencia/Reports sobre design system, sin `window.prompt`; ERSAPS mantiene "registrar ≠ cumplir + validación institucional" (sin claims falsos). Reports (Annual Pack / Reporting) conserva export XLS, impresión y evidencias autenticadas. Suite `compliance-milestone-p`. |
| V — Legacy eradication (en curso) | 🔶 EN PROGRESO | `pending-push` | 20 pantallas migradas a `src/design-system` con paridad funcional en 5 lotes (Security/Audit/Backups/MapView/Users/Tariffs/PeguesContratos/Bodega/FinancialDocuments/Admin/VerifyReceipt/Progress/Metering/Integrations/Settings/DocumentSettings/FieldReadings/Imports/ReceiptVisualStudio/Accounts). `window.prompt` y `.modal` legacy eliminados; fugas de UUID corregidas (FieldReadings, PeguesContratos). `src/components/Layout.tsx` (código muerto) eliminado y 5 suites V2/V3 reapuntadas a `AppShell`. Falta: 3 pantallas subscriber/portal (con R) + gates anti-reintroducción + dedup de rutas admin (S). |
