...
export function SubscriberCards(){
  ...
  const search=useCallback(async(searchQuery=query)=>{try{setResults(await searchSubscribers(searchQuery));setError('')}catch(e){setError((e as Error).message)}},[query]);
    ...
    <div className="subscriber-card-layout">
      ...
      <section>{card?<div className="digital-member-card">
        <div className="member-card-hero">
          <div className="member-card-brand"><span><Waves size={25}/></span><div><strong>Junta de Agua</strong><small>Ficha digital del abonado</small></div></div>
