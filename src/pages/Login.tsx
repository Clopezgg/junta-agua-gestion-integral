...
export function Login(){
 ...
 return <main className="auth auth-premium">
  <section className="auth-showcase">
   ...
   <h1>Gestión integral del servicio comunitario de agua</h1>
   <p>Abonados, conexiones, cobros, caja, gastos, presupuesto, inventario, activos y auditoría en una sola plataforma para la Junta de Agua.</p>
   <div className="auth-security-list">
    <div><ShieldCheck size={20}/><span><strong>Acceso protegido</strong><small>MFA obligatorio y permisos por rol.</small></span></div>
    <div><LockKeyhole size={20}/><span><strong>Datos privados</strong><small>Registros protegidos, archivos privados y auditoría completa.</small></span></div>
    <div><KeyRound size={20}/><span><strong>Cuenta individual</strong><small>No se permiten usuarios compartidos.</small></span></div>
   </div>
   <footer>Junta de Agua · Honduras</footer>
  </section>
  <section className="auth-panel">
   <form className="auth-card" onSubmit={submit}>
    ...
    <label>Correo institucional<div className="input-with-icon"><KeyRound size={16}/><input type="email" autoComplete="email" required value={email} onChange={event=>setEmail(event.target.value)} placeholder="usuario@correo.com"/></div></label>
    <label>Contraseña<div className="input-with-icon"><LockKeyhole size={16}/><input type={showPassword?'text':'password'} autoComplete="current-password" required minLength={8} value={password} onChange={event=>setPassword(event.target.value)} placeholder="Mínimo 8 caracteres"/><button type="button" className="password-toggle" onClick={()=>setShowPassword(value=>!value)} aria-label={showPassword?'Ocultar contraseña':'Mostrar contraseña'}>{showPassword?<EyeOff size={17}/>:<Eye size={17}/>}</button></div></label>
