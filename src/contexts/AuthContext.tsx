...
type AuthValue={
 ...
 permissions:Permission[];mfaVerified:boolean;hasVerifiedFactor:boolean;authError:AuthContextError|null;
...
export function AuthProvider({children}:{children:ReactNode}){
 ...
 const load=useCallback(async(nextSession:Session|null)=>{
  ...
  try{
   ...
   if(subscriberError||!subscriber){if(isNetworkError(subscriberError))setAuthError({kind:'NETWORK_ERROR',message:'No se pudo contactar el servicio del portal.'});else setAuthError({kind:'ACCOUNT_CONTEXT_NOT_FOUND',message:'No existe una cuenta de personal ni un abonado vinculado a su sesión.'});throw subscriberError??new Error('ACCOUNT_CONTEXT_NOT_FOUND');}
 ...
 const value:AuthValue={configured,loading,session,user:session?.user??null,profile,portalSubscriber,portalState,accountKind,permissions,mfaVerified,hasVerifiedFactor,authError,signIn,setPortalSession,refresh,enrollMfa,verifyEnrollment,verifyMfa,signOut,has:(permission)=>permissions.includes(permission)};
