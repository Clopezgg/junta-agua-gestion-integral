...
function fail(e:{message:string}|null){if(e)throw new Error(e.message);}
...
export async function voidPayment(input:{payment_id:string;reason:string}){const {data,error}=await db().rpc('void_payment',{p_payment_id:input.payment_id,p_reason:input.reason});fail(error);return data;}
export async function refundPayment(input:{payment_id:string;amount:number;reason:string}){const {data,error}=await db().rpc('refund_payment',{p_payment_id:input.payment_id,p_amount:input.amount,p_reason:input.reason});fail(error);return data;}
export async function listPayments(limit=100){const {data,error}=await db().rpc('list_payments',{p_limit:limit});fail(error);return data??[];}
export async function listExpenses(){const {data,error}=await db().rpc('list_expenses');fail(error);return data??[];}
export async function createExpenseRequest(input:Record<string,unknown>){const {data,error}=await db().rpc('create_expense_request',{p_payload:input});fail(error);return data;}
export async function approveExpense(id:string,decision:'approved'|'rejected',notes?:string){const {data,error}=await db().rpc('approve_expense',{p_expense_id:id,p_decision:decision,p_notes:notes??null});fail(error);return data;}
